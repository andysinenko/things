...
import com.synenko.things.pdfbook.service.PdfBookService;
...
public class PdfBookController {
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "400", description = "Invalid request parameters or unsupported file format", content = @Content),
    ...
    public ResponseEntity<PdfBookResponse> upload(
            ...
            @Parameter(description = "Language of the book, e.g. EN or RU")
            @RequestParam(value = "language", required = false) String language,

            @Parameter(description = "Title of the book")
            @RequestParam(value = "title", required = false) String title,

            @Parameter(description = "Author ID of the book")
            @RequestParam(value = "author", required = false) Long authorId  // ← было AuthorResponse
    ) throws Exception {
        return ResponseEntity.ok(
                pdfBookService.save(file, category, yearOfRelease, language, title, authorId)
        );
    }
    ...
    @GetMapping("/{id}")
    public ResponseEntity<PdfBook> getOne(
            @Parameter(description = "pdf book id", example = "42", required = true)
    ...
    @Operation(summary = "Download pdf book", description = "Download a PDF file by book ID")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "PDF file successfully returned",
                    content = @Content(mediaType = "application/pdf",
                            schema = @Schema(type = "string", format = "binary"))),
            @ApiResponse(responseCode = "404", description = "Book not found or file does not exist", content = @Content),
            @ApiResponse(responseCode = "500", description = "Internal server error", content = @Content)
    })
    @GetMapping("/{id}/file")
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved list of pdf books",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = PdfBookPageResponse.class))),
            @ApiResponse(responseCode = "500", description = "Internal server error", content = @Content)
    })
    @GetMapping
    public ResponseEntity<PdfBookPageResponse> getAllPdfbooks(
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved list of pdf authors",
                    content = @Content(mediaType = "application/json",
                            array = @ArraySchema(schema = @Schema(implementation = PdfAuthorResponse.class)))),
            @ApiResponse(responseCode = "500", description = "Internal server error", content = @Content)
    })
    @GetMapping("/pdfauthors")
